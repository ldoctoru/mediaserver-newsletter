...
docker run --rm \
    -v ./config:/app/config \
    ghcr.io/seaweedbraincy/jellyfin-newsletter:
```
> [!note]
> Note: It is recommended to use a static version instead of `latest`, and manually upgrade. [Last version](https://github.com/SeaweedbrainCY/jellyfin-newsletter/releases)
...
0 8 1 * * root docker run --rm -v PATH_TO_CONFIG_FOLDER/config:/app/config/ ghcr.io/seaweedbraincy/jellyfin-newsletter:
```
